        ...
        if transcript_text.lower().startswith("hey assistant"):
            ...
            if question:
                ...
                try:
                    ...
                    logger.info(f"🤖 Responding to question (LLM latency: {t_end - t_start:.2f}s)")
                except Exception as e:
                    logger.error(f"❌ Q&A error: {e}")
    
    @agent.events.subscribe
    async def handle_llm_response(event: LLMResponseChunkEvent):
        """Log agent responses and timestamp them for latency analysis"""
        if hasattr(event, 'delta') and event.delta:
            logger.info(f"🤖 Agent delta (t={time.monotonic():.2f}): {event.delta}")
